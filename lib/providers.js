...
async function retryableFetch(url, init, { maxAttempts = 4, baseMs = 400 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      await sleep(baseMs * 2 ** attempt + Math.random() * 150);
      continue;
    }
    if (!RETRYABLE.has(resp.status)) return resp; // 200/4xx -> pass through
    // Estimate a stable per-request id for the "do not retry on the first
    // attempt too aggressively" case; respect a Retry-After if present.
...
// ---------------------------------------------------------------------------
async function complete(model, { prompt, system, temperature = 0.7, maxTokens = 1024 }) {
  ...
  if (provider.mock) {
    await sleep(120);
    return { text: mockStream(model, { prompt }), meta: { latencyMs: 120, tokens: { in: 0, out: 0 }, selectedModel: model.id } };
  }
  ...
  if (provider.type === 'anthropic') {
    ...
    return { text: content, meta: { latencyMs: Date.now() - started, selectedModel: model.id, tokens: { in: data.usage?.input_tokens, out: data.usage?.output_tokens } } };
  }
...
// ---------------------------------------------------------------------------
async function* streamComplete(model, { prompt, system, temperature = 0.7, maxTokens = 1024 }) {
  const provider = providers[model.provider];
  if (!provider) throw new Error(`No provider configured for ${model.provider}`);
  if (!provider.apiKey) throw new Error(`Missing API key for provider "${model.provider}". Set it in your environment.`);
  ...
  if (provider.mock) {
    ...
    yield { done: true, meta: { latencyMs: 0, tokens: { in: 0, out: 0 }, selectedModel: model.id } };
    return;
  }
  ...
  if (provider.type === 'anthropic') {
    ...
    yield { done: true, meta: { latencyMs: Date.now() - started, tokens, selectedModel, text: content, reasoning } };
    return;
  }
